...
class UnignoreEmploymentValidatorSpec extends UnitSpec with MockEmploymentsAppConfig {
  ...
  trait Test {
    ...
    def validate(nino: String = validNino,
                 ...
                 employmentId: String = validEmploymentId): Either[ErrorWrapper, UnignoreEmploymentRequest] =
      new UnignoreEmploymentValidator(nino, taxYear, employmentId, mockEmploymentsConfig).validateAndWrapResult()
  ...
  "validate" should {
    "return a request object" when {
      "valid" in new Test {
        validate() shouldBe Right(UnignoreEmploymentRequest(parsedNino, parsedTaxYear, parsedEmploymentId))
      }
    }
