...
object CustomEmploymentDateValidator extends ResolverSupport {
  ...
  def validator(taxYear: TaxYear): Validator[StringDateRange] = { case (startDate: String, cessationDate: Option[String]) =>
    def validateStartDate(date: LocalDate) =
      (satisfiesMax(taxYear.endDate, RuleStartDateAfterTaxYearEndError)
        .thenValidate(isInRange(StartDateFormatError))(date))

    def validateCessationDate(maybeDate: Option[LocalDate]) =
      (satisfiesMin(taxYear.startDate, RuleCessationDateBeforeTaxYearStartError)
        .thenValidate(isInRange(CessationDateFormatError)).validateOptionally(maybeDate))
