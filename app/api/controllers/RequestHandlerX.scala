...
trait RequestHandlerX[InputRaw <: RawData] {
...
object RequestHandlerX {
  ...
  class ParserOnlyBuilder[InputRaw <: RawData, Input] private[RequestHandlerX](parser: RequestParser[InputRaw, Input]) {

    def withService[Output](
        serviceFunction: Input => Future[Either[ErrorWrapper, ResponseWrapper[Output]]]): RequestHandlerXBuilder[InputRaw, Input, Output] =
      RequestHandlerXBuilder(parser, serviceFunction)

  }

  case class RequestHandlerXBuilder[InputRaw <: RawData, Input, Output] private[RequestHandlerX](
                                                                                                  ...
                                                                                                  resultCreator: ResultCreatorX[InputRaw, Input, Output] = ResultCreatorX.noContent[InputRaw, Input, Output](),
                                                                                                  auditHandler: Option[AuditHandler] = None
  ) extends RequestHandlerX[InputRaw] {

    def handleRequest(rawData: InputRaw)(implicit ctx: RequestContext, request: UserRequest[_], appConfig: AppConfig, ec: ExecutionContext): Future[Result] =
      Delegate.handleRequest(rawData)

    def withResultCreator(resultCreator: ResultCreatorX[InputRaw, Input, Output]): RequestHandlerXBuilder[InputRaw, Input, Output] =
      copy(resultCreator = resultCreator)

    def withErrorHandling(errorHandling: ErrorHandling): RequestHandlerXBuilder[InputRaw, Input, Output] =
      copy(errorHandling = errorHandling)

    def withAuditing(auditHandler: AuditHandler): RequestHandlerXBuilder[InputRaw, Input, Output] =
      copy(auditHandler = Some(auditHandler))

    /** Shorthand for
      * {{{
      * withResultCreator(ResultCreatorX.plainJson(successStatus))
      * }}}
      */
    def withPlainJsonResult(successStatus: Int = Status.OK)(implicit ws: Writes[Output]): RequestHandlerXBuilder[InputRaw, Input, Output] =
      withResultCreator(ResultCreatorX.plainJson(successStatus))

    /** Shorthand for
      * {{{
      * withResultCreator(ResultCreatorX.noContent)
      * }}}
      */
    def withNoContentResult(successStatus: Int = Status.NO_CONTENT): RequestHandlerXBuilder[InputRaw, Input, Output] =
      withResultCreator(ResultCreatorX.noContent(successStatus))

    /** Shorthand for
      * {{{
      * withResultCreator(ResultCreatorX.hateoasWrapping(hateoasFactory, successStatus)(data))
      * }}}
      */
    def withHateoasResultFrom[HData <: HateoasData](
        ...
        writes: Writes[HateoasWrapper[Output]]): RequestHandlerXBuilder[InputRaw, Input, Output] =
      withResultCreator(ResultCreatorX.hateoasWrapping(hateoasFactory, successStatus)(data))

    /** Shorthand for
      * {{{
      * withResultCreator(ResultCreatorX.hateoasWrapping(hateoasFactory, successStatus)((_,_) => data))
      * }}}
      */
    ...
    private object Delegate extends RequestHandlerX[InputRaw] with Logging with RequestContextImplicits {
